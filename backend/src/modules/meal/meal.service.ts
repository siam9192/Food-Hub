...
import { Meal, Review } from "../../prisma-output/client.js";
import { MealWhereInput } from "../../prisma-output/models/Meal.js";
import { GetMealsParams } from "../../types/meal.type.js";
...
const getAllMeals = async ({
  ...
  const mealsWithRating = meals.map((meal) => {
    ...
    const averageRating =
      totalReviews > 0
        ? meal.reviews.reduce((acc:number, rev:Review) => acc + rev.rating, 0) / totalReviews
        : 0;
